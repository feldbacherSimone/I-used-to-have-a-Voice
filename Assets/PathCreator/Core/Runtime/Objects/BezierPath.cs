...
namespace PathCreation {
 ...
 IsClosed = isClosed;
        ...
        ///<param name="isClosed"> Should the end point connect back to the start point? </param>
        ///<param name="space"> Determines if the path is in 3d space, or clamped to the xy/xz plane </param>
        public BezierPath (IEnumerable<Transform> transforms, bool isClosed = false, PathSpace space = PathSpace.xy, bool useLocal = false):
            this (transforms.Select (t => useLocal ? t.localPosition : t.position), isClosed, space) { }
